...
function MainComponent() {
  ...
  const chgCat = (val) => {
    ...
    // 파라미터로 페이지가 변경되며 새로 고쳐짐
    // 이유는 처음 들어온 cat = 카테고리명 때문임!
    // 그래서 리액트 컴포넌트 업데이트 시 상단 url
    // 마지막 cat 키값을 실제 카테고리명으로 업데이트 해준다!
    ...
    // encodeURIComponent -> 'TIME & GEM'의 특수문자 '&'때문에 인코딩처리함!
  ...
  return ( 
    ...
    // 순서 :
    // 1. createContext()를 생성하여 사용할 곳에 파일을 import한다!
    // 2. 최상위 컴포넌트에서 컨텍스트 프로바이더를 셋팅한다
    // 3. value 속성에 공유할 변수/함수를 넣어준다
    // -> 여러개일 경우 중괄호안에 콤마를 사용하여 셋팅
