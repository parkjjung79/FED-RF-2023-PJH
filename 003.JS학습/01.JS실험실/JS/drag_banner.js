...
/***************************************************** 
    ...
    4. 기능 설계:
        ...
        (1) 오른쪽 버튼 클릭시 다음 슬라이드가
            ...
            -330%로 변경시킨다. -> 박스 한개 값이 110% 이기때문에(좌우가운데 이동이기 때문에 110%*3) 
            -> 슬라이드 이동후!!! 
            바깥에 나가있는 첫번째 슬라이드
            li를 잘라서 맨뒤로 보낸다!
...
// 보정값으로 슬라이드 이동파트에서도 써야하므로 함수 바깥에 선언해
// 공유한다!
  ...
  dFn.addEvt(ele, "mouseup", () => {
    ...
    // 왜 ? 자유드래그와 달리 슬라이드는 마지막에 항상 특정위치에 가있기 때문.
...
function goWhere(target){
    ...
    // clientWidth 값 박스의 가로크기 값
    // 220% 이므로 2.2를 곱하면 된다!

    
    console.log('슬라이드LEFT:',tgLeft,
    '\n기준LEFT:',-pointLeft);
    // 역슬래시n '\n'은 console 찍을때 줄바꿈 기능임

    // 3. 방향 판별하기 : 기준값의 특정값을 더하고 뺌
