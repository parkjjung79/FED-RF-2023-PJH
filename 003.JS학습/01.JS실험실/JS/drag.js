...
/*************************************** 
    ...
    -> 모바일 이벤트 : touchmove
    (3) mousemove 에서 차이값을 타겟요소의 left,top값에 반영
...
function goDrag(ele){ // ele - 드래그 대상요소
    ...
    const dMove = (e) => {
        ...
        if(drag){
            ...
            // - pageX,pageY는 일반브러우저용
            // - touches[0].screenX,touches[0].screenY는 터치스크린용
            mvx = e.pageX || event.touches[0].screenX;
            mvy = e.pageY || event.touches[0].screenY;
    ...
    // (5) 마지막 위치 포인트 셋팅함수 : lx,ly
    const lastPoint = () => {
        // 움직일때 위치값을 기존값에 계쏙 더함!
        lx += rx;
        ly += ry;
