...
2. 변경방법:
    ...
    ex) let newList3 = Object.keys(list3).map(v=>list3[v]);
...
// 검색 기준 선택박스(번호/제목/내용박스) 변경시 정렬선택 초기화하기 ///
...
dFn.addEvt(sel4, "change", ()=>{
  // 정렬용 데이터는 원본 list2를 쓰지 않고 newList 사용!
  targetData = newList;
  targetEle = showList4;
}); /////// change ///////
...
function searchingFn(){
  ...
  console.log('검색결과:',res);
    ...
    // 그 변수의 값은 업데이트 한다!
    // 단, 그 변수 데이터는 sorting '정렬'시에 사용하도록 한다
    // -> newList변수
    newList = res;
...
dFn.addEvt(dFn.qs('.sample'),'click',()=>{
  ...
  let res1 = list2.find(v=>{
    ...
    // if(v.tit=='당근마켓에가자') retrun true;
    // 데이터가 완벽하게 일치하지 않으면 undefined 리턴!
  });
console.log("검색어:'당근마켓에 가자'\n결과:",res1);

  // 2. find() 메서드 LIKE 검색하기 : 데이터 일부로 찾음!
  ...
  let res3 = list2.filter(v=>{
    ...
    // if(v.tit.indexOf('샤인머스캣')!=-1)return true;
    // 검색결과가 없으면 반배열이 리턴됨(배열.length==0)
  });
  console.log("검색어:'당근'\n결과",res3);
...
// 2단계 : 키배열을 객체의 값으로 대체하여 원하는 값배열을 만든다
// let myVal = myKey.map(v=>list3[v]);
// console.log('값배열:',myVal);

// 1,2단계를 한 번에 값배열 만들기 : 오브젝트-키쓰-맵!
