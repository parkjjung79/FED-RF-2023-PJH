...
// 달력 컴포넌트 불러오기
import DalCom from "./carlendar.js";

// 달력 컴포넌트 함수 호출하여 달력 셋팅하기!
// 대상: .dalcom, .dalcom2
new DalCom('.dalcom1'); // 임의로 특정공간에 할당

const dc2 = new DalCom('.dalcom2') // 지정한 주소에 할당

// 요구사항: 두 번째 달력은 다음달 달력이 처음에 출력되게함!
// 다음달 달력을 쓸 수 있게 이전/다음달 달력 변경 내부함수를
// 생성자함수에 this키워드로 등록하여 노출해야한다!!

// dc2.chgCalendar(-1); -> 9월부터 시작 괄호안에 숫자로 설정 변경
dc2.chgCalendar(1);
...
calBox.mouseenter(function(){
    ...
    $(this).find('.date').click(()=>{
        ...
        let myData = 
        $(this).next().val();
        // console.log('날짜정보요소:',$(this).next()); -> 확인용

        // 달력 인스턴스생성시 노출된 속성/메서드중
        // addZero() - 한자리 숫자 앞에 0추가가능
        // week[] - 한글 요일 변환 배열
        ...
        dc2.addZero(myData[2])+'월 ('+
        dc2.week[myData[3]]+'요일)'
        
        

        // 출력 : 해당달력의 부모의 이전 text 박스
