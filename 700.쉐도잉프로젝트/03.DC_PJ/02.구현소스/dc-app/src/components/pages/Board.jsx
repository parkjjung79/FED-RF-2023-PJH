    ...
    // 3-4. 글쓰기 서브밋 /////////
      ...
      // 2. 통과시 실제 데이터 입력하기
        ...
        let temp = {
          ...
          att:
          uploadFile.current?
          uploadFile.current.name:'',//파일이름 업데이트
        ...
        // [선택파일 서버전송]
        if(uploadFile.current){ //null아닌 할당상태일때만
...
const AttachBox = ({saveFile}) => { //saveFile 프롭스펑션다운!
  // [상태관리변수] ////////////
  // 1. 드래그 또는 파일을 첨부할때 활성화여부 관리 변수
  // 값: true이면 화성화, false이면 비활성화
  ...
  // 드래그를 할때 dragOver 이벤트는 비활성화함!(필요가없음)
  ...
  */
  ...
  return (
    <label
      ...
      onDrop={controlDrop}
    >
      {/* 파일을 클릭하여 선택창이 뜰 때 파일을 선택하면
      현재상태가 변경되기 때문에 onChage이벤트 속성을 씀! */}
...
// 업로드 표시 아이콘 SVG태그 리턴 컴포넌트 ///////
// 화살표함수에 중괄호 안쓰고 JSX태그를 바로 쓰면 리턴 키워드 생략
const UpIcon = () => (
  <svg>
    {/* viewBox가 중요함 */}
