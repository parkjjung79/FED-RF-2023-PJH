...
function makeGrid(ele,idx) { // ele - 대상요소 / idx -순번(데이터순번)
  ...
  gridData[idx].forEach((val) => {
    ...
    // 1이면 'poster-img'로 셋팅함!
    hcode += `<li>
            <figure>
                <img
                src="images/${idx?'poster_img':'live_photo'}/${val.imgName}.jpg" alt="${val.title}">
...
gnb.forEach((ele) => {
  // 서브메뉴가 있을때만 이벤트 설정하기!
  // if문에서 undefined/null 은 false 처리됨!
...
function showMv(){
 ...
 // 대상 : 나자신(.intro-mv-img)
 this.innerHTML = `
  <video src='./images/pc/intro_mv.mp4' controls autoplay></video>
 `;
...
previewData.sort((x,y)=>{
 ...
 // 변수 -> 문자형을 숫자형으로 바꿈!
 // 배열값 중 idx속성값을 가져와서 숫자형 변환!후 'Number'를 추가!
 // 사용은 Number();
 let a = Number(x.idx);
 let b = Number(y.idx);

 // 배열 순서변경인 메서드인 sort() 내부에 retun값을
