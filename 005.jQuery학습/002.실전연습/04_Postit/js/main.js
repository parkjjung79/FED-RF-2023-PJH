// 제이쿼리 UI 드래스 & 드롭 기능활용하기 ///

// 제이쿼리 코드 구역 : html태그 로딩 후 실행구역
$(()=>{
    ...
    // 3-1. 드래그 시작 이벤트 : dragstart -> 이벤트 이미지 변경
    ...
    $('.dropshow').droppable({
        drop: function(evt,ele){
            ...
            // 그래서 맨뒤에 요소 추가 append() 메서드! 

            // 동영상 아이디 정보는 드래그된 요소(즉, target)의
            // data-mv 속성의 값으로 셋팅되어 있다!
            // 속성값 읽어오기는 선택요소.attr(속성명)
            let mvId = target.attr('data-mv') ;
            console.log('동영상ID:',mvId);

            mbox.append(`<iframe src="https://www.youtube.com/embed/${mvId}?autoplay=1" allow="autoplay" style="width:100%;height:100%;border:none"></iframe>
            `)

            // 9. 동영상 박스 숨기고 1초 후 슬라이드다운(slideDown)으로 보이기
            ...
            .click(function(){ // 닫기버튼 a요소 클릭시
                ...
                mbox.slideUp(1000,function(){
                    // 2. 자기자신 없애기(this는mbox)
                    $(this).remove();
                    // 태그제거 -> 안그럼 박스안지워져서 소리만 계속남 
